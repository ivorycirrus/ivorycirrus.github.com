---
layout: post
title:  "HTTP 상태코드 418 - I'm a teapot"
date:   2016-06-16 12:00
categories: [network , iot]
permalink: /archivers/i-am-a-teapot
---

만일 멀리 떨어진 곳에 있는 커피포트를 원격으로 제어해서 커피를 끓이고자 한다면 어떤 방식으로 통신을 해야 할까?

이 주제에 대해 국제 인터넷 표준화 기구(IETF)에서는 꽤 오래전 HTTP 프로토콜을 이용하여 커피포트를 제어 하는 방법에 대한 확장 프로토콜을 제안한 바가 있다.
물론 만우절 농담을 가장한 반쯤은 장난이 섞인 표준문서지만, IoT라는 단어가 신문/TV뉴스는 물론 일반 제품광고에까지 등장하는 요즘 IoT 서비스를 설계하는데 있어서
어떠한 문제점들을 고려해야 하는지에 대한 좋은 길잡이가 될 수 있다고 본다. 이 글에서는 커피포트를 제어하는 프로토콜인 HTCPCP에 대해 소개하고 이 프로토콜이
현재 IoT서비스의 설계시 어떤 부분에서 참고가 될 수 있는지에 대해 이이기 해보고자 한다.

## 0. 시작하며

웹 브라우저로 인터넷을 이용해 본 경험이 있는 사람이라면 대부분은 아래와 비슷한 페이지를 본 경험이 있을 것이다.<br/><br/>
이 화면은 Google 웹 사이트에서 제공하지 않는 URL로 접속했을 때의 페이지로 '404, That's an error.' 이라는 오류 메세지를 표시하는 것을 볼 수 있다. 이 때, 404라는 숫자는 'Page Not Found'를 뜻하는 HTTP상태코드를 나타낸다. HTTP로 통신을 하는 과정에서 다양한 통신상태에 대한 구분이 필요 했고, 이를 잘 분류된 3자리 숫자로 표현 한 것이 HTTP상태코드이다.<br/><br/>
...
![http404]({{ site.url }}/images/i-am-a-teapot/http404.png)
...
![rfc2324_htcpcp]({{ site.url }}/images/i-am-a-teapot/rfc2324_htcpc.png)
...
![teapot_google]({{ site.url }}/images/i-am-a-teapot/google-teapot-418.png)
...
![418teapot_rasppi]({{ site.url }}/images/i-am-a-teapot/418teapot_rasppi.jpg)
...
[![demo_htcpcp]({{ site.url }}/images/i-am-a-teapot/demo_htcpcp.png)](https://www.youtube.com/watch?v=R8B-mAHWodk)
